...
# ① 오타 정규화 사전 — 자주 나는 오타·표기변형을 표준어로 치환(매칭 전에 적용).
#    실제 오타가 관찰되면 여기 한 줄씩 추가하면 됨.
TYPO_NORMALIZE = {
    '이탇': '이탈', '이탈됨': '이탈', '이탍': '이탈',
    '심암': '심함', '심합': '심함', '심햠': '심함',
    '가싯': '가시', '가싀': '가시', '가시발샹': '가시발생', '가시발셍': '가시발생',
    '윤활재': '윤활제',
    '블로워': '브로워',
    '스크레치': '스크래치', '스크렛치': '스크래치', '스크래찌': '스크래치',
    '광텍': '광택', '광택발셍': '광택발생',
    '튀ㅁ': '튐', '튀임': '튐', '튕': '튐',
    '탈짖': '탈지', '탈지불럄': '탈지불량',
}

# ② 드롭다운(설비불량유형) → 표준분류 정밀 매핑 (공백 제거 키).
#    텍스트(불량내용) 분류가 실패해도 이 값으로 정확히 복구. 와이어 튐/이탈도 구분.
DROPDOWN_MAP = {
    '가시발생': ('표면품질', '가시발생'),
    '탈지불량': ('표면품질', '탈지불량'),
    '스크래치': ('표면품질', '스크래치'),
    ...
    '도유미흡': ('소모품·마모', '도유/보루'),
    '와이어이탈': ('와이어 주행', '와이어 이탈'),
    '와이어올라탐': ('와이어 주행', '와이어 이탈'),
    '와이어튐': ('와이어 주행', '와이어 튐'),
    '와이어꼬임': ('와이어 주행', '와이어 꼬임'),
    '와이어간섭': ('와이어 주행', '와이어 데임/간섭'),
    '권취불량': ('와이어 주행', '권취불량'),
    '선떨림': ('와이어 주행', '선떨림'),
    '냉각불량': ('설비·기계', '냉각 이상'),
    '냉각수누수': ('설비·기계', '냉각 이상'),
    '브로워불량': ('설비·기계', '급기/여과(필터·브로워)'),
    '세라믹볼불량': ('설비·기계', '급기/여과(필터·브로워)'),
    '피막농도기록누락': ('관리·기록', '기록 누락/오류'),
    '선습성불량': ('관리·기록', '선습성/공정조건'),
}


def normalize_text(t):
    """오타 정규화 사전 적용."""
    for wrong, right in TYPO_NORMALIZE.items():
        if wrong in t:
...
def standardize(df):
    ...
    df['txt'] = df['불량내용'].fillna('').astype(str).str.strip().map(normalize_text)  # ① 오타 정규화
    ...
    def _cls(row):
        ...
        for major, minor, pat in RULES:
            if re.search(pat, t):
                return (major, minor)
        # ② 텍스트 분류 실패 → 설비불량유형(드롭다운) 정밀 매핑으로 복구
        old = str(row.get('설비불량유형', '')).strip()
        key = old.replace(' ', '')
        if key in DROPDOWN_MAP:
            return DROPDOWN_MAP[key]
        for k, v in DROPDOWN_MAP.items():   # 드롭다운 표기 흔들림 대비 부분매칭
            if k and k in key:
                return v
        return ('미분류', old or '미분류')
